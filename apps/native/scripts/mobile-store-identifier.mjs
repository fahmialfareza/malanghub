...
function patchIos() {
  ...
  updateFile(
    ...
    (source) => {
      ...
      // Handle self-closing <dict/> generated by some Tauri versions
      let result = source.replace(/<dict\s*\/>/, `<dict>\n${entry}</dict>`);
      // Handle normal <dict>…</dict> regardless of internal whitespace / line endings
      result = result.replace(/<\/dict>(\r?\n)<\/plist>/, `${entry}</dict>$1</plist>`);
      return result;
    },
  );
